...
async def create_bank_agent():
    ...
    agent_graph = create_agent(
        ...
        middleware=[
            ...
            # 🔒 PII Protection
            # Маскирует номера кредитных карт и телефоны в ответах агента
            PIIMiddleware(
                "credit_card",
                strategy="mask",
            ...
            PIIMiddleware(
                "phone_number",
                detector=r"(?:([+]?[78])[-.\s]?)(?:[(](\d{1,3})[)][-.\s]?)?(\d{1,4})[-.\s]?(\d{1,4})[-.\s]?(\d{1,9})",
                strategy="mask",
                apply_to_input=False,
                apply_to_output=True
            ),
            # 🔒 Human-in-the-Loop для критичных операций
            # Требует подтверждения пользователя для open_credit_card и open_deposit

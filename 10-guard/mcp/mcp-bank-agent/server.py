...
def generate_contract_number() -> str:
    """
    Генерация случайного номера договора по вкладу
    
    Формат: D + 8 цифр (например, D12345678)
    """
...
def generate_phone_number() -> str:
    ...
    Использует российский формат с кодом оператора от 900 до 999
    """
    # Коды операторов для мобильных номеров в России
    operator_codes = ['900', '901', '902', '903', '904', '905', '906', '907', '908', '909',
    ...
    subscriber = f"{random.randint(10, 99)}-{random.randint(10, 99)}"
    
    return f"+7 ({operator_code}) {exchange}-{subscriber}"
...
@mcp.tool(
    name="open_deposit",
    description="Открытие нового вклада для клиента с расчетом доходности",
)
async def open_deposit(
    ...
    Рассчитывает доходность по вкладу с классической формулой капитализации.
    Возвращает номер договора, номер телефона (заглушки) и детальные данные вклада.
    ...
    Returns:
        Форматированная информация о новом вкладе с расчетами
    """
    logger.info(f"💰 open_deposit called: amount={amount}, rate={rate}, term={term_months}")
    
    # Локальный импорт для работы с датами
    from datetime import datetime, timedelta
    
    # Генерируем заглушки - номер договора и телефон
    contract_number = generate_contract_number()
    phone_number = generate_phone_number()
    
    # Рассчитываем доходность с ежемесячной капитализацией
    income, total, breakdown = calculate_compound_interest(
        amount=amount,
        rate=rate,
        term_months=term_months,
        capitalization_months=1  # Ежемесячная капитализация
    )
    
    # Вычисляем дату окончания вклада
    start_date = datetime.now()
    end_date = start_date + timedelta(days=term_months * 30)  # Приблизительно 30 дней в месяце
    ...
    result = (
        ...
        f"Дата открытия: {start_date.strftime('%d.%m.%Y')}\n"
        f"Дата окончания: {end_date.strftime('%d.%m.%Y')}\n\n"
        "**Расчет доходности:**\n"
        f"Доход по вкладу: {income:,.2f} руб.\n"
        f"Итоговая сумма: {total:,.2f} руб.\n"
        f"Эффективная доходность: {((total/amount - 1) * 100):,.2f}%\n\n"        
        f"Детали отправлены на номер {phone_number}\n"
    )
    
    logger.info(f"Deposit opened successfully: {contract_number} for {amount:,.0f} rub")

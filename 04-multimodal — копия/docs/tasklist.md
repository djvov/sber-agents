...
| 7 | pending | ⏳ | Извлечение потребленных и потраченных калорий из текстового сообщения
...
- [ ] Итерация 7 — Извлечение потребленных и потраченных калорий из текстового сообщения
  - Цель теста: бот извлекает записи о калориях и возвращает `CalorieResponse` — объект с полями `calories: list[CalorieExtractionResult]` (каждая запись содержит `date`, `time`, `calorie_type`, `kkal`, `category`) и `answer: str` (текстовый ответ ассистента)
  - Тестовый пример: сообщение "сегодня днем покатался на велосипеде на 560 ккал"
{
  "calories": [
    {
      "date": "2024-10-30",

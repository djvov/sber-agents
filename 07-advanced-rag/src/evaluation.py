...
def create_ragas_embeddings():
    ...
    if provider == "openai":
        ...
        logger.info(f"Creating RAGAS ollama embeddings: {config.RAGAS_EMBEDDING_MODEL}")
        return OllamaEmbeddings(model=config.RAGAS_EMBEDDING_MODEL)
...
def init_ragas_metrics():
    ...
    # Настройки для выполнения с rate limiting
    run_config = RunConfig(
        max_workers=1,  # Уменьшаем параллелизм для предотвращения слишком частых запросов
        max_wait=180,
        max_retries=3
    )
...
def evaluate_dataset(dataset_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    def target(inputs: dict) -> dict:
        ...
        # Rate limiting для RAG запросов (синхронная задержка)
        time.sleep(config.EVALUATION_RAG_DELAY)
    ...
    for i, result in enumerate(client.evaluate(
        target,
        data=dataset_name,
        evaluators=[],
        ...
        # Задержка между обработкой результатов LangSmith
        if i > 0:  # Не задерживаем первый элемент
            time.sleep(config.EVALUATION_LANGSMITH_DELAY)
    ...
    for idx, run_id in enumerate(run_ids):
        ...
        # Задержка между feedback запросами в LangSmith
        if idx > 0:  # Не задерживаем первый элемент
            time.sleep(config.EVALUATION_LANGSMITH_DELAY)
        
        for metric_idx, metric in enumerate(ragas_metrics):
            if metric.name in row:
                score = row[metric.name]
                
                # Задержка между метриками для одного run_id
                if metric_idx > 0:
                    time.sleep(config.EVALUATION_METRIC_DELAY)
